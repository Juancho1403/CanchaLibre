...
import { reservasConfirmadas } from '../../../../Services/Propietario'


export default function ConfirmadaPropietario() {
    const { id } = useParams();
    ...
    const cargarCanchas = async (dia) => {
        ...
        try {
            let data;
            if (dia) {
                data = await reservasConfirmadas(id, mifecha);
            } else {
                data = [];
            }
            setHorarios(data)
    ...
    return (
        <div className="flex flex-col items-center ">
            <NavBar />
            <div>
                <h1 className="text-2xl font-bold mb-10 mt-10">Selecciona un día de la semana que desea saber la cantidad de horarios confirmados</h1>
            </div>
